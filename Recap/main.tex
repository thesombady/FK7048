...
that the constants infront of the terms is indenpendent of $x$. If the root are real and distinct, then the solution is given by, for a second order problem: In this instance it doesn't matter if the roots are real, plugging them back into the problem will yield a cosinus and sinus expansion.
\begin{align*}
    y(x) = Ae^{r_1x} + Be^{r_2x}.
...
\begin{itemize}
    ...
    \item \textbf{Irregular singular points}: where $p(x)$ and $q(x)$ approaches $\infty$, but where $(x-x_0)p(x)$ and $(x-x_0)^2q(x)$ also does.
\end{itemize}
...
indicial equation, which are used to substitute the unknown $r$ in the series-expansion. Frobenius method can only be applied to regular singular points whilst the power series ansats can be applied to both regular and irregular singular points.
...
\begin{align*}
    \colorboxed{red}{\oint_{\Gamma} f(z)dz = 2\pi i \sum_{i} \res{f}{z_i},}
...
\begin{align*}
    ...
    &=\sum_{i}\int_{\partial\Gamma_i} f(z)dz.
...
\end{align*}
\subsubsection*{Example}
Suppose the following integral:
\begin{align*}
    \int_0^\infty \frac{dx}{x^3 x+ 1}dx.
\end{align*}Suppose one solves this by construcing a path-integral from $(0,0)\times(R,0)\times(re^{2i\theta/3})$ which is closed $(\Gamma)$, then $z^3 = r^3e^{i\theta3}$.
Using this one has the following:
\begin{align*}
    \oint_\Gamma\frac{1}{z^3 + 1}dz &= \lim_{R\to\infty}\int_0^{R}\frac{1}{x^3 + 1}dx + \int_{arc} + \int_{R}^0\frac{e^{2i\pi/3}}{z^3e^{2i\pi}}\\
    &= (1-e^{i2\pi/3})\int_0^\infty \frac{dx}{x^3 + 1} = 2\pi i\sum_{i}\res{f}{z_i}\\
    &= \frac{2\pi}{3\sqrt{3}}
\end{align*}
...
\begin{align*}
    \colorboxed{red}{f(z) = f(0) + \sum_{i = 1}^Na_i \left(\frac{z_i}{z - z_i} + \frac{1}{z_i}\right)}.
...
\end{center}
\begin{align*}
    \colorboxed{red}{\psi(z) =\frac{d}{dz}\left(\ln(\Gamma(z))\right) = \frac{\Gamma'(z)}{\Gamma(z)}}
\end{align*}
...
\end{align*}
\begin{align*}
    &\colorboxed{red}{\int g'(x)\cdot h(x) dx = \left[g(x)\cdot h(x)\right] - \int g(x)\cdot h'(x)dx}\\
    &\colorboxed{blue}{\int_0^\infty t^ne^{-st}dt = \frac{\Gamma(n+1)}{s^{n+1}}}
\end{align*}
\newpage
\input{exam.tex}
